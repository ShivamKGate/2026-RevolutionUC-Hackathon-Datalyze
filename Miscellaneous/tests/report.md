...
This document summarizes the **orchestrator pipeline runtime master plan** (see repository plan file), what was built, how it behaves under **live API testing**, and a structured **accuracy / coverage view** (expectations vs reality). It is the single narrative index for orchestrator work; raw timestamped evidence lives under the folder described below.
...
| `Miscellaneous/tests/report.md`           | This file — plan alignment, scores, expectations vs actual         | **Tracked** (recommended) so the team keeps a stable narrative                                                          |
